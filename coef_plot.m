% Files from your simulations
files = {"D:\F1 comparison report\MCL39 data\coefficients\20ms.csv";
    "D:\F1 comparison report\MCL39 data\coefficients\30ms.csv";
    "D:\F1 comparison report\MCL39 data\coefficients\40ms.csv";
    "D:\F1 comparison report\MCL39 data\coefficients\50ms.csv";
    "D:\F1 comparison report\MCL39 data\coefficients\60ms.csv";
    "D:\F1 comparison report\MCL39 data\coefficients\70ms.csv";};
cutoffTime = 200;   % time (s) after which averages are calculated
...
for i = 1:numel(files)
    ...
    % Average after cutoffTime
    mask = (t >= cutoffTime);
    avgCL_val = mean(CL(mask),'omitnan');
    avgCD_val = mean(CD(mask),'omitnan');

    % Extract freestream velocity from filename (e.g. "20ms.csv" → 20)
...
% Sort results by freestream velocity
[speeds, idx] = sort(speeds);
avgCL = avgCL(idx);
avgCD = avgCD(idx);
eff   = eff(idx);

% --- Plot CL and CD vs freestream velocity ---
...
% --- Plot aerodynamic efficiency vs freestream velocity ---
figure('Color','w'); hold on; grid on;
